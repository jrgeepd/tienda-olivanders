...
def snapshot(items):
    return [(i.name, i.sell_in, i.quality) for i in items]


def make_shop_day0():
    shop = GildedRose()
    shop.addItem(NormalItem("+5 Dexterity Vest", 10, 20))
    shop.addItem(AgedBrie("Aged Brie", 2, 0))
    shop.addItem(NormalItem("Elixir of the Mongoose", 5, 7))
    shop.addItem(Sulfuras("Sulfuras, Hand of Ragnaros", 0, 80))
    shop.addItem(Sulfuras("Sulfuras, Hand of Ragnaros", -1, 80))
    shop.addItem(BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 15, 20))
    shop.addItem(BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 10, 49))
    shop.addItem(BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 5, 49))
    shop.addItem(ConjuredItem("Conjured Mana Cake", 3, 6))
    return shop


def test_golden_master_all_days_in_order():
    shop = make_shop_day0()

    for day, expected in enumerate(EXPECTED):
        assert snapshot(shop.getItems()) == expected, f"Mismatch on day {day}"
        shop.updateDate()
